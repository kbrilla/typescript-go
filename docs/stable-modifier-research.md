# Stable Modifier for Function Types - Research & Design Document
...
### Related Issues
- [TypeScript #60948](https://github.com/microsoft/TypeScript/issues/60948) - `stable` modifier proposal
...
| **TypeScript** | No (proposed) | `stable` (proposed) | Proposed for #60948 |
...
| Keyword | Pros | Cons |
|---------|------|------|
| `stable` | Matches mathematical stable property; clearly states the function call "is" its value | Could be confused with stable function `x => x`; not immediately clear to non-math users |
...
We recommend **`stable`** over `stable` for these reasons:
...
3. **Less confusion**: `stable` could be confused with the stable function pattern (`x => x`), while `stable` has no such ambiguity
...
However, following the original proposal's terminology and avoiding bikeshedding, we'll use **`stable`** as proposed in the issue, since that's what the community is discussing.

### Adjudicated Outcome (Current)
Final decision for this proposal stage:
- Keep `stable` now for proposal continuity and implementation/docs alignment.
- Defer any rename decision to an explicit upstream naming checkpoint.

Why this was adjudicated:
- Upstream continuity: active issue/proposal discussion currently uses `stable`.
...
### Edge Case 1: Stable function called with arguments
```typescript
// Should produce a type error — stable is only for parameterless calls
declare const fn: stable (x: string) => number; // ERROR
```
**Decision**: Error when `stable` is on a function type with parameters.
...
```typescript
declare let value: stable () => string | undefined;
if (value() !== undefined) {
...
```typescript
declare const value: stable () => string | number;
...
### Edge Case 7: Stable on class methods
```typescript
class Signal<T> {
  stable getValue(): T { ... } // Not supported initially
}
```
**Decision**: For initial implementation, only support on function type syntax (`stable () => T`).

### Edge Case 8: Stable with generic types
```typescript
...
### Syntax
```typescript
// Function type with stable modifier
...
// With generic
declare function createSignal<T>(initial: T): stable () => T;
```

### Semantics
1. The `stable` modifier can only appear on function types with **no parameters**
2. Calls to stable functions are treated as **stable references** for type narrowing
...
1. **AST**: Add `KindStableKeyword` as contextual keyword, `ModifierFlagsStable` flag
2. **Scanner**: Map `"stable"` to `KindStableKeyword`
3. **Parser**: Allow `stable` modifier on function types
4. **Checker**: Add `SignatureFlagsStable`, implement flow analysis for stable calls
5. **Flow Analysis**: Update `isMatchingReference` for call expressions, update cache key generation
6. **Grammar Checks**: Validate stable modifier placement and constraints
7. **Printer**: Emit `stable` modifier on function types

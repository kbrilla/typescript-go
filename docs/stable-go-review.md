# Stable Modifier PR — Go Code Review

**Reviewer**: Senior Go Engineer  
**Scope**: All Go source changes in stable CFA modifier PR  
...
**[INFO]** Package-level `stableBoundaryPreserveRulesByKind` map at `flow.go` ~L46 is initialized once and read-only afterward. No issue.
...
**[INFO]** All stable-related code operates on the `*Checker` receiver, which is single-goroutine scoped. The `reportedStableBoundaryDiagnostics` set at `checker.go` ~L881 is used exclusively within checker methods. No concurrent access concern.
...
**[INFO]** Diagnostic emission in `reportStableBoundaryInvalidationDiagnostic` properly deduplicates via the `reportedStableBoundaryDiagnostics` set. Good pattern.
...
**[INFO]** `stableBoundaryKind` and `stableBoundaryPreserveRuleID` use `int8` as underlying type — good choice for small enums, compact.

**[INFO]** `SignatureFlagsStable` is correctly added to `SignatureFlagsPropagatingFlags`, ensuring stable semantics survive generic instantiation.

**[INFO]** The `switch` in `shouldPreserveStableBoundaryByRule` has a `default: return false` — safe exhaustive handling.

**[MEDIUM]** `isStableCallReference` calls `getResolvedSignature` with `CheckModeTypeOnly` — expensive but cached. Called from multiple hot paths. The `isNoArgCallExpression` guard at the top of `classifyStableBoundary` mitigates this significantly since most references won't be call expressions.
...
- `stableBoundaryKind`, `stableBoundaryPreserveRuleID` — lowercase unexported types, no stutter
- `classifyStableBoundary`, `shouldPreserveStableBoundaryNarrowing` — verb-leading method names
- `isStableCallReference`, `isAliasEscapeAssignmentForCallReference` — `is` prefix for boolean returns

**[INFO]** Stable-specific functions are colocated near the flow analysis functions they integrate with. Good organization.

**[INFO]** Constants use consistent naming with project patterns (e.g., `stableBoundaryKindUnknownCall` mirrors `AssignmentKindCompound`).
...
Map lookup + iteration + switch dispatch through `shouldPreserveStableBoundaryByRule` adds indirection. The switch in `shouldPreserveStableBoundaryByRule` negates most table-driven benefit.
...
**[INFO]** Benchmark test `stable_bench_test.go`:
...
**[LOW]** Package-level `stableCFABenchmarkDiagCount` as compiler sink is functional but slightly unconventional. Not a bug.
...
`isStableCallReference` is called from `classifyStableBoundary` (called from `getTypeAtFlowCall` and `getTypeAtFlowAssignment`), `shouldReportStableBoundaryInvalidationDiagnostic`, and multiple preserve rule functions. Each invocation calls `c.getResolvedSignature(reference, nil, CheckModeTypeOnly)`.

Additionally, `stableBoundaryInvalidationDiagnosticMessage` calls `classifyStableBoundary` a second time when the caller already computed the boundary kind.

**Mitigation**: The guard `isNoArgCallExpression(reference)` at the top of `classifyStableBoundary` is an effective early-exit — most references won't be call expressions. `getResolvedSignature` is internally cached by the checker.

**Recommendation**: Thread the already-computed boundary kind to diagnostic functions to avoid re-classification. Consider caching the `isStableCallReference` result on the node if profiling shows it as a bottleneck.
...
**[INFO]** Stable CFA code integrates cleanly into existing `getTypeAtFlowNode` dispatch:
- In `getTypeAtFlowAssignment`: stable boundary checks placed after `containsMatchingReference` — exactly where additional reference-invalidating checks belong
- In `getTypeAtFlowCall`: stable boundary checks placed after effects-signature processing — correct position
...
```
Minimal, correct extension. Only matches zero-argument calls (stable contract).
...
| 5 | LOW | checker.go ~L19866 | `stableBoundaryInvalidationDiagnosticMessage` re-classifies boundary already computed by caller | Pass pre-computed `kind` as parameter |
| 6 | LOW | flow.go ~L290-304 | `classifyStableBoundary` called twice for same node (CFA + diagnostic) | Thread boundary kind to diagnostic path |
...
2. **Effective early exits** — `isNoArgCallExpression` at the top of nearly every stable function ensures non-stable references short-circuit immediately.
...
5. **Diagnostic deduplication** — `reportedStableBoundaryDiagnostics` set prevents duplicate diagnostics. Clean pattern.

6. **Day-one benchmarks** — Having benchmarks from the start is excellent engineering practice for a CFA feature.

7. **Modifier flag integration** — `ModifierFlagsStable` at bit 17 fits naturally. `SignatureFlagsStable` in propagating flags ensures generic instantiation correctness.

8. **Grammar validation defense-in-depth** — Stable constraint enforced at parse time AND grammar-check time.

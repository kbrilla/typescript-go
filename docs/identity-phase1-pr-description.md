...
| Unrelated call boundary exemption | Standalone and method calls unrelated to identity receiver; PropertyAccess/Identifier callee check; same-receiver method calls remain boundaries; reordered classification so unrelated-call check runs before callback detection, fixing expression-statement trivial passthrough false positives | Low | Eliminates false positive TS100015/TS100014 for function calls matching getter transparency; significant error reduction across test suite | ✅ IMPLEMENTED — `isUnrelatedCallForIdentityReference` exempts all calls unrelated to identity endpoint. Reordered classification: unrelated call check now runs before callback detection, fixing expression-statement trivial passthrough false positives. Error reductions: Closures 4→0, P8Conservative 2→0, ValueTypeBoundary 6→0, HeuristicDiagnostics 7→0, Boundaries 18→3, GetterParitySweep 8→2, MultiArgCallback 4→0, Parity 8→6, SignalPatterns 2→1, Tier2 14→6, InOperator 2→0 |

# Stable + Heuristic CFA TDD Plan (Phase 1)

## Scope
This plan intentionally excludes `mutator` and `invalidates` syntax/semantics for now.
...
Naming lock note:
- Phase 1 uses `stable` as the fixed term for all implementation, tests, and diagnostics in this branch.
...
- INV-04 Contract dependency boundary: contract-dependent effects (`mutator`/`invalidates`, constrained post-call narrowing) are out of Phase 1 runtime behavior.
...
- repeated `stable` reads should narrow after guard
...
## Step 2: Stable Parse + Bind (Green for metadata)
Implement parser and binder support for `stable` only.
...
## Step 3: Stable Read Reuse in Checker (Green)
...
- Added checker micro-bench coverage in `internal/checker/stable_bench_test.go`.
- Covered scenarios:
...
- `stable` narrowing works in targeted local-flow scenarios.
...
- remaining hard cases are documented for the final explicit-contract phase (`mutator`/`invalidates`).

## Phase 1 Readiness Additions (Doc/Test Planning Only)

### R1: Constrained-Overload Readiness Pack
Why this is not Phase 1 behavior:
- It depends on explicit `mutator`/`invalidates` contracts, which are deferred to the final phase.
...
- Do not narrow multiple endpoints unless explicit invalidates resolve to a unique set.
...
- `stableModifierConstrainedOverloadExplicitContracts.ts` positive case: selected constrained overload narrows to `U`.
- Negative control: unconstrained overload in same API does not narrow.
- Negative control: unresolved/ambiguous invalidates do not narrow and emit ambiguity diagnostic.
...
## Final Phase (Prepared, Deferred): Explicit `mutator`/`invalidates`
...
- [x] Step 1: Baseline failure capture (red)
  - Added and ran local stable tests to establish failing behavior first.
- [x] Step 2: Stable parse + bind (core)
  - Parser/binder support for `stable` is active.
...
- [x] Step 3: Stable read reuse in checker (core)
  - Repeated-read narrowing now works for core direct stable call patterns.
...
- [x] Step 4: Tier 1 heuristic invalidation (covered slices)
  ...
  - Closed the P5 getter-vs-stable write parity slice for same-receiver `read()` then `set(non-nullish)` shape.
  - Added Tier 1 write-form expansion slice for matching endpoint property/element compound and unary writes, plus safe bracket mutator parity (`stableModifierTier1Writes.ts`).
...
- [x] Step 6: Getter/setter parity sweep expansion
  - Added dedicated local parity test coverage in `stableModifierParity.ts` for repeated-read success, callback boundary invalidation, await boundary invalidation, write-call analog invalidation, and one-liner ternary shape.
  - Added discriminated-union stable parity coverage for kind-guard narrowing and post-unknown-call invalidation.
  - Added comprehensive getter-to-stable parity visibility sweep in `stableModifierGetterParitySweep.ts` with categorized sections (repeated reads, branch merges, callback/await, write invalidation, aliasing, ternary, nested access).
  - Added broad submodule-derived getter-to-stable parity corpus in `stableModifierGetterCorpus.ts` with source-traceable section labels and intentional mismatch visibility baselines.
  ...
  - Added missing getter-origin parity expansion matrix in `testdata/tests/cases/compiler/stableModifierGetterMissingMatrix.ts`.
...
- [ ] Step 7: Tier 2 guarded invalidation and precision (broad)
  ...
  - Remaining: implement guarded precision preservation for broader local consumer/passthrough forms when receiver stable and non-mutating forwarding can be proven.
- [x] Step 8: Diagnostics for heuristic limits (narrow boundary slice)
  - Added boundary guidance diagnostic emitted when stable narrowing is conservatively dropped at uncertainty boundaries.
  - Covered by `stableModifierHeuristicDiagnostics.ts` for unknown call, callback, await, and alias-escape shapes.
- [x] Step 9: Performance guardrails + broad regression sweep (micro-bench baseline)
  - Added deterministic checker micro-bench coverage for repeated reads and uncertainty boundaries in `internal/checker/stable_bench_test.go`.
...
- Bug: When an interface has both `stable (): T` and `(v: T): void` overloads, calling with zero args selects the setter overload instead of the stable overload. Produces 6 spurious errors in `stableModifierParity.ts` (lines 73, 75, 80, 82).
- Action: Write a red test demonstrating the overload selection bug (zero-arg call on hybrid interface resolves to setter instead of stable overload), then fix checker overload resolution to prefer the stable signature for zero-arg calls.

### Phase 1 Parity Gap Closure Candidates (from baseline parity analysis)

- ~~*Independent stable endpoints:*~~ **Done.** Stable calls are now recognized as pure reads — `b()` no longer invalidates unrelated `a()` narrowing.
- *Await boundary (`await delay()`) and assignment-expression callback boundary:* Document as Phase 2 parity targets. Getter survives `await delay()` and assignment-expression callbacks; stable does not. Only `await Promise.resolve()` is currently preserved.

### New Test Coverage To Add (from baseline parity analysis)

| Pattern | Priority | Description |
|---|---|---|
| Destructured stable signals | HIGH | `const [value, setValue] = createSignal()` and `const { data, isSuccess } = useQuery()` — aliased discriminant narrowing via destructuring |
| Class member stable | HIGH | `this.read()` in class methods where `read` is an stable member — common Angular signal pattern |
| `typeof stable().kind` discriminant | MEDIUM | `typeof obj.kind === "string"` pattern and optional chain variant `typeof obj?.kind` |
| Closure past last assignment | MEDIUM | After last assignment, closures should see narrowed type from stable read |
| `stable()!` non-null assertion | LOW | `stable()!` should narrow like `getter!` |

### Stale Comments Found (minor cleanup)

- `stableModifierParity.ts` ~line 95: comment says "should error" after `unknownShapeMutate()` but baseline shows NO error (correct preserve).
- `stableModifierGetterParitySweep.ts` ~line 69: comment says "error (conservative boundary)" for empty callback `invoke(() => {})` but baseline shows NO error (correct preserve).

### Latest Increment (Missing Getter-Origin Matrix)
- Added `testdata/tests/cases/compiler/stableModifierGetterMissingMatrix.ts` to cover missing getter-origin parity scenarios requested by sweep/swarm:
...
- Targeted run and acceptance:
  - `go test -run='TestLocal/stableModifierGetterMissingMatrix\.ts' ./internal/testrunner`
...
- Validation evidence:
  - `go test -run='TestLocal/stableModifierGetterMissingMatrix\.ts' ./internal/testrunner` passed after baseline acceptance with `M6` closed.
  - `go test -run='TestLocal/stableModifierBoundaries\.ts' ./internal/testrunner` remained intentionally conservative for non-ambient unknown-call and callback/await/alias boundaries.
- Safety check outcome:
  ...
  - explicit negative control added: unknown call with args still invalidates narrowing in `stableModifierBoundaries.ts`.
- Decision:
  - landed the checker rule with bounded guards (ambient, no-arg, `void`, expression-statement call boundary, stable read with union return).
...
- `testdata/tests/cases/compiler/stableModifierBoundaries.ts`
- `testdata/tests/cases/compiler/stableModifierTier1Writes.ts`
- `testdata/tests/cases/compiler/stableModifierTier2.ts`
- `testdata/tests/cases/compiler/stableModifierGetterParitySweep.ts`
- `testdata/tests/cases/compiler/stableModifierGetterCorpus.ts`
...
| --- | --- | --- | --- |
| Compound assignment (dot) invalidation | `if (model.value !== undefined) { model.value += "!"; const s: string = model.value; }` | Implemented (invalidates) | `testdata/tests/cases/compiler/stableModifierTier1Writes.ts` |
| Logical assignment (dot) invalidation | `??=`, `||=`, `&&=` on `model.value` after guard | Implemented (invalidates) | `testdata/tests/cases/compiler/stableModifierTier1Writes.ts` |
| Logical assignment (bracket-literal) invalidation | `model["value"] ||= ...` after guard | Implemented (invalidates) | `testdata/tests/cases/compiler/stableModifierTier1Writes.ts` |
| Unary mutation (dot) invalidation | `model.value++`, `--model.value` after guard | Implemented (invalidates) | `testdata/tests/cases/compiler/stableModifierTier1Writes.ts` |
| Unary mutation (bracket-literal) invalidation | `model["value"]++` after guard | Implemented (invalidates) | `testdata/tests/cases/compiler/stableModifierTier1Writes.ts` |
| Bracket-literal mutator parity with dot-form | `store["set"]("next")` vs `store.set("next")` | Implemented (preserve parity in narrow `read`/`set(non-nullish)` slice) | `testdata/tests/cases/compiler/stableModifierTier1Writes.ts` |
| Bracket-literal simple assignment parity with dot-form | `model["value"] = ...` vs `model.value = ...` | Covered parity check (current behavior preserved) | `testdata/tests/cases/compiler/stableModifierTier1Writes.ts` |
...
- Added red tests in `stableModifierBoundaries.ts` and `stableModifierGetterParitySweep.ts` for:
...
- Red->green evidence:
  - red: `go test -run='TestLocal/(stableModifierBoundaries|stableModifierGetterParitySweep)\.ts' ./internal/testrunner`
...
- Added/updated TDD test shapes:
  - parity/corpus target shape now expects preserved narrowing in `stableModifierGetterParitySweep.ts` and `stableModifierGetterCorpus.ts`
  - conservative control shape in `stableModifierP8Conservative.ts` keeps `unknownMutateWithArg(1)` as a boundary drop
- Red->green evidence:
  - red: `go test -run='TestLocal/(stableModifierGetterCorpus|stableModifierGetterParitySweep|stableModifierP8Conservative)\.ts' ./internal/testrunner`
  - green: `npx hereby baseline-accept` then same targeted rerun

### Latest Increment (Parity Gap Closure - P6)
- Closed `P6` in `stableModifierGetterParitySweep.ts` for the narrow direct alias shape:
  - `if (stableBasic() !== undefined) { const stableAlias = stableBasic; const s: string = stableBasic(); }` is now parity-matched.
- Checker change (`internal/checker/flow.go`): narrowed alias-escape invalidation to keep direct `const` alias declarations of stable call endpoints non-invalidating by themselves.
...
- Updated boundary expectations for the direct const alias shape in `stableModifierBoundaries.ts`.
- Red->green evidence:
  - red: `go test -run='TestLocal/stableModifierGetterParitySweep\.ts' ./internal/testrunner`
  - green: baseline acceptance + focused stable suite rerun (`stableModifierGetterParitySweep`, `stableModifierBoundaries`, `stableModifierParity`, `stableModifierTier2`, `stableModifierGetterCorpus`).

### Latest Increment (Parity Gap Closure - P4)
- Closed `P4` in `stableModifierGetterParitySweep.ts` for a narrow await-safe statement shape:
  - `if (stableBasic() !== undefined) { await Promise.resolve(); const s: string = stableBasic(); }` is now parity-matched.
- Checker change (`internal/checker/flow.go`): added a narrowly scoped preserve rule in flow-call invalidation for:
  - stable call references,
...
- Conservatism retained for broader await boundaries (`await delay()`, `const x = await delay()`) covered in `stableModifierBoundaries.ts`.
- Conservatism retained for broader await boundaries outside the narrow preserve shapes, including await-assignment forms (`const x = await delay()`) covered in `stableModifierBoundaries.ts`.
- Red->green evidence:
  - red: `go test -run='TestLocal/stableModifierGetterParitySweep\.ts' ./internal/testrunner`
  - green: baseline acceptance + targeted stable suite re-run.

### Latest Increment (Parity Gap Closure - P3)
- Closed `P3` in `stableModifierGetterParitySweep.ts` for the narrow no-op callback statement shape:
  - `if (stableBasic() !== undefined) { invoke(() => {}); const s: string = stableBasic(); }` is now parity-matched.
- Checker change (`internal/checker/flow.go`): added a narrowly scoped preserve rule in flow-call invalidation for:
  - stable call references,
...
- Updated callback-boundary local tests to use non-empty callback bodies where conservative invalidation is still expected:
  - `stableModifierBoundaries.ts`
  - `stableModifierParity.ts`
  - `stableModifierHeuristicDiagnostics.ts`
...
- Kept stable Phase 1 improvements that remain green without regressions:
  ...
  - binder: call-expression participation in narrowing references/flow attachment for stable usage
  - checker: call-expression truthiness narrowing fallback when no type predicate applies
- Stable targeted local test set:
  - `stableModifierErrors.ts`
...
- Added `testdata/tests/cases/compiler/stableModifierDiagnostics.ts` with stable grammar/placement checks:
  - parameterized stable function type (TS100012)
  - duplicate stable modifier (TS1030)
  - stable modifier on constructor type position (TS1184)
- Parser lookahead now recognizes stable-modifier trails for function/constructor type starts in declaration type positions.
- Stable-focused local suite now green:

### Latest Increment (Tier 2 Narrow Precision)
- Added a Tier 2 positive case in `testdata/tests/cases/compiler/stableModifierTier2.ts` for inline trivial passthrough forwarding:
  - `const fwd = ((x) => x)(read);` does not invalidate prior `stable` narrowing.
...
- Checker change is intentionally narrow and syntactic in `internal/checker/flow.go`:
  ...
  - exempt expression-statement passthrough calls only when the helper target is a trivial local passthrough function-like shape and the single argument is the exact stable read endpoint
...
- Remaining Tier 2 gaps:
  ...
  - `stableModifierErrors.ts`
  - `stableModifierNarrowing.ts`
  - `stableModifierDiagnostics.ts`

### Latest Increment (Getter Corpus X3 Assessment)
- Inspected `X3` exact shape in `stableModifierGetterCorpus.ts` and confirmed baseline diagnostics remain:
...
- New targeted guidance (TS100015) now appears on unknown-call boundary invalidation sites:
  - "Stable narrowing was conservatively dropped after an unknown call. Extract the guarded value to a local temporary before the call to preserve precision."
- Existing generic uncertainty-boundary guidance (TS100014) remains in place for callback and `await` boundary drops.
- Diagnostics-focused coverage was extended in `stableModifierHeuristicDiagnostics.ts`, including a duplicate-suppression shape to ensure one boundary diagnostic per unknown call node.

### Latest Increment (Boundaries - Narrow Slice)
- Added `testdata/tests/cases/compiler/stableModifierBoundaries.ts` for expression-statement call boundaries.
...
- Checker change (`internal/checker/flow.go`): conservative invalidation of stable-call narrowing across non-matching flow-call boundaries.
...
- Extended `testdata/tests/cases/compiler/stableModifierBoundaries.ts` with a conditional initializer callback-boundary case:
...
- Targeted red/green evidence:
  - red: `go test -run='TestLocal/stableModifierBoundaries\.ts' ./internal/testrunner`
  - green: `npx hereby baseline-accept` then re-run targeted stable local suite

### Latest Increment (Parser Disambiguation Stability)
- Fixed a parser ambiguity where `stable<...>` in type-reference positions was being misclassified as the start of an stable function type.
- Parser change (`internal/parser/parser.go`): `nextTokenStartsStableFunctionOrConstructorType` now requires an unambiguous function-type parameter list before accepting `stable` + `<...>` as function-type syntax.
- This removes the submodule regression in `declarationEmitMappedTypePreservesTypeParameterConstraint.ts` while preserving Phase 1 stable tests.
...
- Added `testdata/tests/cases/compiler/stableModifierParity.ts` as a dedicated local parity test for issue-family patterns.
...
- Red/green completed with targeted run:
  - red: `go test -run='TestLocal/stableModifierParity\.ts' ./internal/testrunner`
  - green: accepted only `stableModifierParity` baselines and re-ran targeted stable suite

### Latest Increment (Discriminated-Union Stable Parity)
- Extended `testdata/tests/cases/compiler/stableModifierParity.ts` with a discriminated-union shape:
...
- Red phase was run first:
  - `go test -run='TestLocal/stableModifierParity\.ts' ./internal/testrunner`
...
- Baseline + validation:
  - accepted relevant parity baselines (`stableModifierParity.errors.txt`, `stableModifierParity.symbols`, `stableModifierParity.types`)
  - re-ran targeted stable suite and required full `hereby` validation (`build`, `test`, `lint`, `format`)

### Latest Increment (Boundaries - Await Slice)
- Extended `testdata/tests/cases/compiler/stableModifierBoundaries.ts` with an `await` uncertainty-boundary scenario.
...
- Accepted only relevant baselines for this slice:
  - `testdata/baselines/reference/compiler/stableModifierBoundaries.errors.txt`
  - `testdata/baselines/reference/compiler/stableModifierBoundaries.symbols`
  - `testdata/baselines/reference/compiler/stableModifierBoundaries.types`

### Latest Increment (Boundaries - Alias Escape Slice)
- Extended `testdata/tests/cases/compiler/stableModifierBoundaries.ts` with a minimal alias-escape scenario (`const escapedRead = read`).
- Red phase: `go test -run='TestLocal/stableModifierBoundaries\.ts' ./internal/testrunner` failed with boundary baseline mismatch before checker updates.
...
- Accepted only relevant boundary baselines and revalidated the targeted stable suite.

### Latest Increment (Boundaries - Alias Escape Reassignment Case)
- Extended `stableModifierBoundaries.ts` with a second alias-escape form using binary assignment (`reassignedRead = read`).

### Latest Increment (Tier 1 Write-Form Parity Expansion)
- Extended `testdata/tests/cases/compiler/stableModifierParity.ts` with small write-form parity scenarios:
...
- Accepted only relevant parity baselines:
  - `testdata/baselines/reference/compiler/stableModifierParity.errors.txt`
  - `testdata/baselines/reference/compiler/stableModifierParity.symbols`
  - `testdata/baselines/reference/compiler/stableModifierParity.types`
...
- Accepted updated boundary baselines and re-verified the focused stable local suite:
  - `stableModifierErrors.ts`
  - `stableModifierNarrowing.ts`
  - `stableModifierDiagnostics.ts`
  - `stableModifierBoundaries.ts`
...
- Extended `testdata/tests/cases/compiler/stableModifierBoundaries.ts` with `const x = await delay()` and a post-await read assertion.
- Red phase confirmed mismatch with targeted run:
  - `go test -run='TestLocal/stableModifierBoundaries\.ts' ./internal/testrunner`
...
- Extended `testdata/tests/cases/compiler/stableModifierBoundaries.ts` with `const result = invoke(() => {})` and a post-call read assertion.
- Red phase confirmed mismatch with targeted run:
  - `go test -run='TestLocal/stableModifierBoundaries\.ts' ./internal/testrunner`
...
- Extended `testdata/tests/cases/compiler/stableModifierBoundaries.ts` with an indirect helper callback argument form:
...
- Extended `testdata/tests/cases/compiler/stableModifierBoundaries.ts` with helper passthrough alias escape:
...
- Targeted stable suite is green after baseline acceptance:
  - `stableModifierErrors.ts`
  - `stableModifierNarrowing.ts`
  - `stableModifierDiagnostics.ts`
  - `stableModifierBoundaries.ts`
...
- Added `testdata/tests/cases/compiler/stableModifierTier2.ts` as a narrow starter test for candidate Tier 2 shapes:
...
- TDD flow for this increment:
  - red first: `go test -run='TestLocal/stableModifierTier2\.ts' ./internal/testrunner`
  - accepted only `stableModifierTier2` baselines
  - green rerun of the same test and targeted stable suite including the new file
...
- Chosen low-risk mismatch: `P5` write parity in `stableModifierGetterParitySweep.ts`.
...
- Red phase (tests first):
  - Updated expectations in:
    - `testdata/tests/cases/compiler/stableModifierGetterParitySweep.ts`
    - `testdata/tests/cases/compiler/stableModifierParity.ts`
  - Targeted run failed as expected:
    - `go test -run='TestLocal/(stableModifierGetterParitySweep|stableModifierParity)\.ts' ./internal/testrunner`
...
- Expanded `stableModifierBoundaries.ts` with strict const no-op callback alias cases for:

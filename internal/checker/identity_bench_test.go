...
var identityCFABenchmarkDiagCount int

const identityCFARepeatedReadsBenchmarkSource = `
...
const identityCFAUncertaintyBoundaryBenchmarkSource = `
declare const read: identity () => string | undefined;
...
func BenchmarkIdentityCFAFlow(b *testing.B) {
	b.Run("RepeatedReads", func(b *testing.B) {
		benchmarkIdentityCFASemanticDiagnostics(b, identityCFARepeatedReadsBenchmarkSource)
	})

	b.Run("UncertaintyBoundary", func(b *testing.B) {
		benchmarkIdentityCFASemanticDiagnostics(b, identityCFAUncertaintyBoundaryBenchmarkSource)
	})
}

func benchmarkIdentityCFASemanticDiagnostics(b *testing.B, sourceText string) {
	...
	identityCFABenchmarkDiagCount = len(warmDiags)
	...
	for b.Loop() {
		...
		identityCFABenchmarkDiagCount = len(diags)
	}
}

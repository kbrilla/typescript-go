...
// =============================================================================
// Signal-pattern test coverage for identity modifier CFA parity.
...
// -----------------------------------------------------------------------------
// Section 1: Destructured identity signals (SolidJS-style)
// -----------------------------------------------------------------------------

// Signal-style tuple return
type ReadonlySignal<T> = identity () => T;
...
// Narrowing after destructured identity call
if (value() !== undefined) {
    const s: string = value(); // should narrow to string
    s;

    // setValue is an independent variable, not linked in Phase 1.
    // Calling setValue should NOT invalidate identity narrowing on value.
    setValue(undefined);
...
// -----------------------------------------------------------------------------
// Section 2: Class member identity patterns
// -----------------------------------------------------------------------------

// Class with identity property-style members (supported in Phase 1)
declare class ReactiveStore {
    user: identity () => { name: string } | undefined;
    count: identity () => number | null;
...
// -----------------------------------------------------------------------------
// Section 3: typeof identity() patterns
// -----------------------------------------------------------------------------
...
declare const mixedUndef: identity () => string | number | undefined;
...
// -----------------------------------------------------------------------------
// Section 4: Non-null assertion identity()! patterns
// -----------------------------------------------------------------------------

declare const maybe: identity () => string | undefined;

const definite = maybe()!; // should be string
const definiteLen = maybe()!.length; // should work

declare const maybeNum: identity () => number | null;
...
// Array identity — no narrowing needed, just type correctness
comp.items().forEach(item => {
    item.toUpperCase(); // should work — items() returns readonly string[]
});

// Array identity — length check

//// [tests/cases/compiler/stableModifierDeclarationParity.ts] ////

//// [stableModifierDeclarationParity.ts]
...
//// [stableModifierDeclarationParity.js]
"use strict";
// === Section 1: stable function declaration ===
stable function getValue() {
    return Math.random() > 0.5 ? "hello" : undefined;
}
if (getValue() !== undefined) {
    const v = getValue(); // should be narrowed to string
}
// === Section 2: mutator function declaration ===
let state = "initial";
mutator function setState(val) {
    state = val;
}
stable function getState() {
    return state;
}
if (getState() !== undefined) {
    const s = getState(); // narrowed
    setState(undefined); // mutator — should invalidate getState() narrowing
    const s2 = getState(); // back to string | undefined
}
// === Section 3: stable function expression ===
const getX = stable function () {
    return Math.random() > 0.5 ? 42 : null;
};
if (getX() !== null) {
    const x = getX(); // narrowed
}
// === Section 4: mutator function expression ===
const setX = mutator function (val) { };
stable function readX() { return null; }
if (readX() !== null) {
    const r = readX(); // narrowed
}
// === Section 5: stable arrow function ===
const getY = stable () => {
    return Math.random() > 0.5 ? "world" : undefined;
...
if (getY() !== undefined) {
    const y = getY(); // narrowed
}
// === Section 6: mutator arrow function ===
const setY = mutator (val) => { };
stable function readY() { return undefined; }
if (readY() !== undefined) {
    const r = readY(); // narrowed
    setY(undefined); // mutator call
    const r2 = readY(); // back to union
}
// === Section 7: stable async arrow function (combined modifiers) ===
const getAsync = stable async () => {
    return undefined;
};
// === Section 8: stable get accessor in class ===
class Counter {
    ...
    mutator set count(value) {
        this._count = value;
    }
}
// Error: stable with parameters (function declaration)
stable function badStableWithParams(x) { return x; } // Error
// Error: stable on constructor
class BadConstructor {
    stable constructor() { } // Error
}
...
var NS;
(function (NS) {
    stable function getVal() {
        return "ns";
    }
    NS.getVal = getVal;
    mutator function setVal(v) { }
    NS.setVal = setVal;
})(NS || (NS = {}));
if (NS.getVal() !== undefined) {
    const v = NS.getVal(); // narrowed
}
